...
function TestimonialCard({ t }) {
  ...
  return (
    <div className="flex-shrink-0 w-80 sm:w-96 bg-white rounded-2xl border border-gray-100 shadow-sm p-7 flex flex-col">
...
export default function TestimonialsSection() {
  ...
  // ── Scroll the track without touching window scroll ──────────────────────
  const scrollTrackTo = useCallback((i) => {
    const track = trackRef.current
    if (!track) return
    const card = track.children[i]
    if (!card) return

    // Use scrollLeft on the track element itself — never scrollIntoView
    const cardLeft    = card.offsetLeft
    const trackWidth  = track.offsetWidth
    const cardWidth   = card.offsetWidth
    const targetLeft  = cardLeft - (trackWidth / 2) + (cardWidth / 2)
  ...
  // ── Auto-advance: ONLY updates index + scrolls the track ─────────────────
  useEffect(() => {
    if (paused) return
    const timer = setInterval(() => {
      setIndex(prev => {
        const next = (prev + 1) % total
        scrollTrackTo(next)   // scroll the track div, not the page
        return next
      })
    }, 4500)
    return () => clearInterval(timer)
  }, [paused, total, scrollTrackTo])
  ...
  return (
    <section id="testimonials" className="py-24 bg-gray-900 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {/* Track — overflow scroll lives here, not on window */}
          <div
            ref={trackRef}
